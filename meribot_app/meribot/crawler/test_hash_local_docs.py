...
def main():
    fname = "DefinicionDeCompetenciasCCA.pdf"
    fpath = os.path.join(DOCS_DIR, fname)
    ext = os.path.splitext(fname)[1].lower()
    if not os.path.isfile(fpath) or ext not in EXTS:
        print(f"No se encontró el archivo {fname} o la extensión no es soportada.")
        return
    print(f"\nProcesando: {fname}")
    ...
    # Consulta directa tras el upsert: obtener todos los embeddings del documento
    ...
    # Recuperar todos los ids que correspondan a este documento
    all_results = collection.get()
    ids = all_results.get('ids', [])
    docs = all_results.get('documents', [])
    metas = all_results.get('metadatas', [])
    filtered = [ (i, d, m) for i, d, m in zip(ids, docs, metas) if i.startswith('DefinicionDeCompetenciasCCA.pdf_chunk_') ]
    print(f"[POST-UPSERT] Recuperados del doc: {len(filtered)}")
    for idx, (id_, doc, meta) in enumerate(filtered):
        print(f"  {idx+1}: ID={id_} | Texto={doc[:60]}... | Metadatos={meta}")
